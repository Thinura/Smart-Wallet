...
class DetailViewController: UIViewController,  NSFetchedResultsControllerDelegate, UITableViewDelegate, UITableViewDataSource, UIPopoverPresentationControllerDelegate{
    ...
    func configureCell(_ cell: ExpenseTableViewCell, withExpense expense: Expense, index: Int) {
        if let category = selectedCategory{
            ...
            print("category.monthlyBudget \((expense.amount/category.monthlyBudget)*100)")
            cell.categoryCellInit(expense.name ?? "",progressValue: (expense.amount/category.monthlyBudget)*100, expenseBudget: expense.amount,startDate: expense.startDate ?? now,dueDate: expense.endDate ?? now, notes: expense.note ?? "")
        }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if selectedCategory == nil{
            categoryDetailView.isHidden = true
            expenseTableView.setEmptyMessage("Add a new Project to manage Tasks", UIColor.black)
            return 0
        }

        if sectionInfo.count == 0{
            editButtonItem.isEnabled = false
            expenseTableView.setEmptyMessage("No tasks available for this Project", UIColor.black)
        }
